...
<html>
  <head>
		...
		<script type="text/javascript">
			function findErrorType(id){
				
				location.href="findByPageError.action?subject.id="+id+"&nowpage="+'1';
			}
			function pageform(page){
				
				var subid = document.getElementById("zk").value;
				location.href="findByPageError.action?subject.id="+subid+"&nowpage="+page;
			}
		</script>
				...
				<div class="row">
					<div class="col-xs-3">
						<ul class="nav default-sidenav">
							...
							<li class="active">
								<a > <i class="fa fa-bar-chart-o"></i> 历史错题 </a>
							</li>
							<li>
					...
					<div class="col-xs-9">
						<div class="page-header">
							<h1><i class="fa fa-history"></i>历史错题</h1>  
							<h10>
							<select onchange="javascript:findErrorType(this.value)" id="zk">
								<option value="-1" >所有</option>
								<c:forEach var="sub" items="${subjectList}">
									<option value="${sub.id}" 
									<c:if test="${subid!=null and sub.id==subid}">
									 selected="selected"
									</c:if>
									>${sub.sname}</option>
								</c:forEach>
						...
						<div class="page-content row">
							<div id="question-list">
								<table class="table-striped table">
									<thead>
										<tr>
											<td width="80px">试题类型</td><td width="120px">科目名称</td><td align="center" width="200px">题目</td><td width="80px"></td>
										</tr>
									</thead>
									<tbody>
										<c:forEach items="${errorList}" var="error">
											<tr>
												<td>
													${error.question.quetype.tname}
												</td>
												<td>
													${error.question.subject.sname}
												</td>
												<td align="center">
													<c:if test="${error.question.qtitle.length() >= 18 }">
													${error.question.qtitle.substring(0,18)}...
													</c:if>
													<c:if test="${error.question.qtitle.length() <= 18 }">
													${error.question.qtitle}
													</c:if>
												</td>
												<td><a href="noteQuestion.action?qid=${error.question.id}">转到此题</a></td>
