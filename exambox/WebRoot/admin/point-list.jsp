...
<body>
<form action="findByPageAndSubjectPoint.action" method="post" name="sform" id="listform">
<input type="hidden" name="nowpage" value="1"/> 
  <div class="panel admin-panel">
    <div class="panel-head"><strong class="icon-reorder"> 内容列表</strong> <a href="" style="float:right; display:none;">添加字段</a></div>
    <div class="padding border-bottom">
      <ul class="search" style="padding-left:10px;">
        <li> <a class="button border-main icon-plus-square-o" href="admin/add-point.jsp"> 添加内容</a> </li>
        
        <li>
       <select name="subject.id" class="input" style="width:200px; line-height:17px;" onchange="change()">
      ...
      <tr>					
        ...
        <tr>
		 ...
		 <tr>									
          ...
          <td>
		   <c:if test="${item.pex.length() >= 12}">${item.pex.substring(0,12)}...</c:if>
		   <c:if test="${item.pex.length() < 12}">${item.pex}</c:if>
		 </td>
          
          <td>
          <div class="button-group"> 
          <a class="button border-main" href="findonePoint.action?pid=${item.id }"><span class="icon-edit"></span> 修改</a>
          </div>
          </td>
         </tr>
         </c:forEach>
